...
export default defineRule({
	...
	async run(context) {
		await context.evaluate(({ report }) => {
			...
			const isRenderableElement = (el: Element | null): el is Element => {
			...
			const isSiblingFillingGap = (
				...
				const filled = { top: false, right: false, bottom: false, left: false }
				const parent = container.parentElement
				if (!parent) return filled

				// Check each sibling
				for (const sibling of parent.children) {
					if (!isRenderableElement(sibling)) continue
